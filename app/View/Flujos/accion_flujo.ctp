...
<script>
  if ($(window).width() <= 750) {
      $('#accion_flujo').load('<?php echo $this->Html->url(array('action' => 'accion_flujo_m', $idFlujo)) ?>', function () {

      });
  } else {
      $('#accion_flujo').load('<?php echo $this->Html->url(array('action' => 'accion_flujo_d', $idFlujo)) ?>', function () {
