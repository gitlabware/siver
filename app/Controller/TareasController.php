...
class TareasController extends AppController {
    ...
    public function tablero() {
        ...
        $sql3 = "(SELECT ('$icono_procesos_estado') AS icono, procesos_estados.created, CONCAT('Proceso Estado') AS tipo, CONCAT('<b>',IF(ISNULL(users.nombre_completo),'Sistema',users.nombre_completo),'</b> marco como ',procesos_estados.estado,' el proceso: <a href=$com ../Procesos/ver_proceso/',procesos_estados.flujos_user_id,'/',procesos_estados.proceso_id,'$com>',procesos.nombre,'</a> de: ',flujos_users.expediente) AS contenido FROM procesos_estados LEFT JOIN users ON(users.id = procesos_estados.user_id) LEFT JOIN procesos ON(procesos.id = procesos_estados.proceso_id) LEFT JOIN flujos_users ON(flujos_users.id = procesos_estados.flujos_user_id) WHERE 1 ORDER BY procesos_estados.created DESC LIMIT 20)";
        $sql4 = "(SELECT ('$icono_flujo') AS icono, flujos_users.created, CONCAT('Flujos Usuario') AS tipo, CONCAT('<b>',users.nombre_completo,'</b>',' creo el flujo: <a href=$com ../Flujos/enflujo/',flujos_users.id,'$com>',flujos_users.expediente,'</a>') AS contenido FROM flujos_users LEFT JOIN users ON(users.id = flujos_users.user_id) WHERE 1 ORDER BY flujos_users.created DESC LIMIT 20)";
    ...
    public function tarea($idFlujoUser = null, $idProceso = null, $idTarea = null) {
        ...
        if (!empty($idTarea)) {
            ...
            $p_estado = $this->ProcesosEstado->find('first', array(
                ...
                'fields' => array('ProcesosEstado.created', 'DATE(ProcesosEstado.created) AS creado','ProcesosEstado.estado'),
                'order' => array('ProcesosEstado.id DESC')
            ));
            //debug($p_estado);exit;
            if ($p_estado['ProcesosEstado']['estado'] === 'Activo' && !empty($proceso['Proceso']['tiempo']) && !empty($proceso['Proceso']['tipo_dias'])) {
                //debug($proceso['Proceso']['tiempo']);exit;
                $nuevafecha = $this->requestAction(array('controller' => 'Procesos', 'action' => 'get_fecha_final', $p_estado[0]['creado'], ($proceso['Proceso']['tiempo']), $proceso['Proceso']['tipo_dias']));
                if ($nuevafecha >= date('Y-m-d')) {
                    $this->request->data['Tarea']['fechafin'] = $nuevafecha;
                }
            }
            $this->request->data['Tarea']['fechainicio'] = date('Y-m-d');
        }
        //debug($proceso);exit;
    ...
    public function creaalertas() {
        ...
        foreach ($tareas as $ta) {
            ...
            $d_alerta['mensaje'] = 'La Tarea ' . $ta['Tarea']['descripcion'] . '  (del proceso ' . $ta['Proceso']['nombre'] . ' en el flujo ' . $ta['FlujosUser']['expediente'] . ') termino su tiempo (' . $ta['Tarea']['fecha_fin'] . ")";
